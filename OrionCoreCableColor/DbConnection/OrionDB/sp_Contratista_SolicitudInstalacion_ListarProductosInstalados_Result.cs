//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace OrionCoreCableColor.DbConnection.OrionDB
{
    using System;
    
    public partial class sp_Contratista_SolicitudInstalacion_ListarProductosInstalados_Result
    {
        public int fiIDContratista_SolicitudInstalacion_Detalle { get; set; }
        public Nullable<int> fiIDContratista_SolicitudInstalacion { get; set; }
        public Nullable<int> fiIDEquifax { get; set; }
        public Nullable<int> fiIDSolicitud { get; set; }
        public Nullable<int> fiIDProducto { get; set; }
        public Nullable<decimal> fnCantidad { get; set; }
        public string fcProducto { get; set; }
        public string fcSerie { get; set; }
        public string fcMac { get; set; }
        public Nullable<int> fiIDInventario { get; set; }
    }
}
