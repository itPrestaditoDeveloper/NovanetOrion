...
{
    public class RenovacionController : BaseController
    {
        ...
        public JsonResult LsitaSolicitudes()
        {
            ...
            using (var conetion = new ORIONDBEntities())
            {
                ...
                var lista = EnviarListaJson(conetion.sp_Solicitudes_BandejaRenovacion(IDRol, IDUser).Where(x=>x.fiIDEstado == 2).ToList());
        ...
        [HttpGet]
        public JsonResult LsitaSolicitudesPendientes()
        {
            ...
            using (var conetion = new ORIONDBEntities())
            {
                var IDRol = GetUser().IdRol;
                var IDUser = GetIdUser();
                var lista = EnviarListaJson(conetion.sp_Solicitudes_BandejaRenovacion(IDRol, IDUser).Where(x => x.fiIDEstado == 0 || x.fiIDEstado == 1).ToList());
                //EnviarNotificacion("DESDE BASE SOLICITUDES");
                return lista;
            }
        ...
        [HttpPost]
        public JsonResult AplicarRenovacion(int IDSolicitud,string Nombre,string Telefono)
        {
            using (var context = new ORIONDBEntities())
            {
                using (var security = new OrionSecurityEntities())
                {
                    try
                    {
                        var Internet = context.sp_InternetRenovacion(IDSolicitud).FirstOrDefault();
                        ...
                        string mensaje = "su solicitud de cambio de velocidad de  "+ Internet.FcInternetPrimero + " MB a " + Internet.FcInternetRenovacion + " MB ha sido aplicado exitosamente.\r\nAgradecemos su confianza y quedamos a su disposición para cualquier consulta adicional ";

                        MensajeriaApi.EnviarSMSPersonalizado(Nombre, Telefono, mensaje);
                        var result = context.sp_OrionSolicitud_aplicarRenovacion(IDSolicitud, GetIdUser()) > 0;

                        return EnviarResultado(true, "Renovacion, aplicada");
        ...
        [HttpGet]
        public ActionResult ConfrimacionNumero(string Nombre, string Telefono, int IDFirma, int IDCliente, int fiIDSolicitudRenovacion, int IdSolicitudes = 0)
        {
            using (var conetion = new ORIONDBEntities())
            {
                ...
                using (var connection = (new ORIONDBEntities()).Database.Connection)
                {
                    ...
                    command.CommandText = $"EXEC sp_OrionSolicitudes_InformacionDocumentacion_Renovacion {fiIDSolicitudRenovacion} ,{IDCliente},{1}";
                ...
                return PartialView(new SolicitudesViewModel { IDSolicitud = IdSolicitudes, Nombre = Nombre, Telefono = Telefono, IDFirma = IDFirma, IdCliente = IDCliente, DatosContrato = infoDocumentos , fiIDSolicitudRenovacion = fiIDSolicitudRenovacion });
            }
        }
        ...
        public ActionResult ModalDetalleCliente(string Nombre, string Telefono, int IDSolicitud)
        {
            ...
            using (var connection = (new ORIONDBEntities()).Database.Connection)
            {
                ...
                connection.Open();
                var command = connection.CreateCommand();
                command.CommandText = $"EXEC sp_OrionSolicitud_Detalle_ClienteListar_Renovacion {GetIdUser()} , {IDSolicitud}";
                using (var reader = command.ExecuteReader())
                {
                    var db = ((IObjectContextAdapter)new ORIONDBEntities());
        ...
        [HttpGet]
        public ActionResult ModalEnviarLinkPago(string Nombre, string Telefono, int IDFirma, string Correo)
        {

            using (var conetion = new ORIONDBEntities())
            {
                return PartialView(new SolicitudesViewModel { Nombre = Nombre, Telefono = Telefono, IDFirma = IDFirma, fcCorreo = Correo });
            }
        ...
        public async Task<JsonResult> MensajeToken(SolicitudesViewModel model, string Token)
        {
            using (var context = new ORIONDBEntities())
            {
                using (var security = new OrionSecurityEntities())
                {
                    try
                    {
                        var result = context.sp_CrearTokenFirma_Renovacion(model.IDFirma, Token) > 0;
                        var Internet = context.sp_InternetRenovacion(model.fiIDSolicitudRenovacion).FirstOrDefault();

                        //MensajeriaApi.EnviarNumeroToken(model.Nombre, model.IDFirma, model.Telefono, Token);
                        //MensajeDeTexto.EnviarNumeroToken(model.Nombre, model.IDFirma, model.Telefono, Token);
                        string mensaje = "NOVANET Agradece su confianza. Le adjuntamos Link, para la firma y documentación del incremento de velocidad en su paquete de "+ Internet.FcInternetPrimero + " MB a "+ Internet.FcInternetRenovacion + " MB ";

                        MensajeriaApi.EnviarSMSPersonalizado(model.Nombre, model.Telefono, mensaje);
                        MensajeriaApi.EnviarSMSPersonalizado(model.Nombre, model.Telefono, "https://ptdto.com/docsRenovacion/Forms/SolitudFirma/SolicitudContratoFirma.aspx?" + model.fiIDSolicitudRenovacion );
                        var _emailTemplateService = new EmailTemplateService();
                        await _emailTemplateService.SendEmailToSolicitudRenovacion(new EmailTemplateServiceModel
                        {
                            CustomerEmail = model.DatosContrato.fcCorreo,
                            //CustomerEmail = "kevinsanme@gmail.com",
                            IDSolicitud = model.fiIDSolicitudRenovacion,
                        ...
                        return EnviarResultado(true, "Token", "Mensaje enviado");
            ...
            using (var context = new ORIONDBEntities())
            {
                using (var security = new OrionSecurityEntities())
                {
                    try
                    {
                        ...
                        modelCorreo.Body = LinkPago;
                        modelCorreo.EmailName = "Novanet";

                        MensajeriaApi.EnviarLinkPago(model.Nombre, model.IdCliente, model.Telefono, LinkPago, model.fcCorreo,"");
                        //MensajeDeTexto.EnviarLinkPago(model.Nombre, model.IdCliente, model.Telefono, LinkPago, model.fcCorreo);
        ...
        [HttpPost]
        public async Task<JsonResult> EnviarSolicitudContratista(SolicitudesViewModel model, string Comentario, int idAgencia, int idAgenciaContratista, string CodigoCliente, string NumeroOrden, string NumeroOrdenTrabajo, DateTime FechaInstalacionAsignada)
        {
            using (var context = new ORIONDBEntities())
            {
                using (var security = new OrionSecurityEntities())
                {
                    try
                    {
                        ...
                        var modelCorreo = new SendEmailViewModel();
                        var CuerpoComentarioCorreo = "Estimado : " + datosCliente.fcNombreEmpresa + " , se le notifica la solicitud de instalacion para el cliente: " + model.Nombre + "<br/>" + " comentario:" + Comentario;
            ...
            using (var context = new ORIONDBEntities())
            {
                using (var security = new OrionSecurityEntities())
                {
                    try
                    {
                        ...
                        //  {
                        //    return EnviarResultado(false, "", "El codigo de promocion");
                        // }
